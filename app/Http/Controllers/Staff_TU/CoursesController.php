...
class CoursesController extends Controller
{
    ...
    {
        // Rate limiter untuk manajemen kursus (serupa admin)
        $this->middleware('throttle:30,1')->only(['store', 'update', 'destroy', 'toggleStatus']);
        $this->middleware('throttle:20,1')->only(['addStudents', 'removeStudent']);
        $this->middleware('throttle:60,1')->only(['index', 'manage']);
    }
    ...
    public function index(Request $request)
    {
        ...
        return view('staff_tu.courses', compact('courses', 'teachers', 'academicYears'));
    }
    ...
    public function toggleStatus(ClassModel $course)
    {
        ...
        return redirect()->route('staff_tu.courses.index')->with('success', $message);
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('staff_tu.courses.index')->with('success', 'Kelas berhasil ditambahkan.');
    }
    ...
    public function update(Request $request, ClassModel $course)
    {
        ...
        return redirect()->route('staff_tu.courses.index')->with('success', 'Kelas berhasil diperbarui.');
    }

    public function destroy(ClassModel $course)
    {
        $course->delete();
        return redirect()->route('staff_tu.courses.index')->with('success', 'Kelas berhasil dihapus.');
    }
    ...
    public function manage(ClassModel $course)
    {
        ...
        return view('staff_tu.courses.manage', compact('course', 'students'));
    }
